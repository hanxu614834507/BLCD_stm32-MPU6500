...
static void BLDC_TIMx_GPIO_Config(void) 
{
  ...
  GPIO_InitStructure.GPIO_Pin =  BLDC_TIM_CH1_PIN;			//PE9
  ...
  GPIO_PinAFConfig(BLDC_TIM_CH1_PORT,BLDC_TIM_CH3_PINSOURCE,GPIO_AF_TIM1); //GPIOE13复用为定时器1
	GPIO_PinAFConfig(BLDC_TIM_CH1N_PORT,BLDC_TIM_CH1N_PINSOURCE,GPIO_AF_TIM1); //GPIOE8复用为定时器1
	GPIO_PinAFConfig(BLDC_TIM_CH2N_PORT,BLDC_TIM_CH2N_PINSOURCE,GPIO_AF_TIM1); //GPIOE10复用为定时器1
	GPIO_PinAFConfig(BLDC_TIM_CH3N_PORT,BLDC_TIM_CH3N_PINSOURCE,GPIO_AF_TIM1); //GPIOE12复用为定时器1
	/* 配置定时器通道2输出引脚模式 */
  GPIO_InitStructure.GPIO_Pin =  BLDC_TIM_CH2_PIN;			//PE11
  GPIO_Init(BLDC_TIM_CH2_PORT, &GPIO_InitStructure);

  /* 配置定时器通道3输出引脚模式 */
  GPIO_InitStructure.GPIO_Pin =  BLDC_TIM_CH3_PIN; 			//PE13
...
static void BLDC_TIMx_Configuration(void)
{
  ...
  TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High;//输出极性比较高
  TIM_OCInitStructure.TIM_OCNPolarity= TIM_OCNPolarity_High;//设置互补极性比较高
  ...
  /* 自动输出启用，中断，死时间，锁配置*/
  TIM_BDTRInitStructure.TIM_OSSRState = TIM_OSSRState_Enable;//在运行模式下“关闭状态”使能
  ...
  TIM_BDTRInitStructure.TIM_DeadTime = 0;//死区时间
